...
if ( ! class_exists( 'snwp_Main' ) ) {
	 ...
	 */
	class snwp_Main extends snwp_Module {
		...
		protected function __construct() {
            ...
            snwp_Settings::get_options();

            if(snwp_Main::$settings['snwp_initial_dt'] == ''){
                $options = snwp_Main::$settings;
                $options['snwp_initial_dt'] = time();
                snwp_Settings::update_options($options);
            }

			$this->register_hook_callbacks();
			$this->modules = array(
				'snwp_Show'     => snwp_Show::get_instance(),
			);
		}
        ...
        public function admin_init() {
            ...
            add_action('wp_ajax_snwp_set_support_link', array( 'snwp_Main', 'ajax_set_support_link'));
            add_action('wp_ajax_snwp_set_support_time', array( 'snwp_Main', 'ajax_set_support_time'));
        }
        ...
        public static function markup_settings_header() {
            ...
            if ( current_user_can( self::REQUIRED_CAPABILITY ) ) {
                $variables = array();
                $variables['setting_logo_path'] = dirname(snwp_Main::$plugin_url) . '/img/logo/wp_directory_wizard_logo.png';
        ...
        static public function ajax_set_support_link(){
            ...
            {
                $options['snwp_author_linking'] = $_POST['state'];
            }
            else{
                $options['snwp_author_linking'] = '1';
            }
            $options['snwp_initial_dt'] = time();
            snwp_Settings::update_options($options);
            die();
